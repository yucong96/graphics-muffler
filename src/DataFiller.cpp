...
DataFiller::DataFiller(const UtilTools &_tools) : tools(_tools) {
...
int DataFiller::saveData(const string filename, const vector<set<int>> &vec_set_int) {
	ofstream output(filename.c_str());
	if (!output.is_open()) {
		return ERR_FILE_OPEN;
	}
	...
	for (int i = 0; i < vec_set_int.size(); i++) {
		output << i << ":\t";
		for (set<int>::iterator it = vec_set_int[i].begin(); it != vec_set_int[i].end(); it++) {
			output << *it << " ";
		}
		output << endl;
	}
...
int DataFiller::saveData(const string filename, const vector<ElementType> &vec_type) {
	ofstream output(filename.c_str());
	if (!output.is_open()) {
		return ERR_FILE_OPEN;
	}
