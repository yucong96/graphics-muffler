...
CoeffItem::CoeffItem() {
  coeffitem_inited = false;
}

void CoeffItem::set_nodenum_and_tools(const int _n_num, const UtilTools* _tools) {
  tools = _tools;
  n_num = _n_num;
...
void CoeffItem::gradient_dot_norm(const Volume &v, const Face &surf_f,
				  const double multiplier, const int is_normalize, const int flag) {
  assert(coeffitem_inited);
...
void CoeffItem::laplace(const vector<Volume> &v_set, const vector<Face> &f_set,
  ...
  for (set<VolumeIndex>::iterator it = ring_1_v[n].begin(); it != ring_1_v[n].end(); it++) {
	...
	for (set<FaceIndex>::iterator it = ring_1_f[n].begin(); it != ring_1_f[n].end(); it++) {
		...
		gradient_dot_norm(near_v, opposite_f, 1.0 / ring_1_v_volume, AREA_NORMALIZE, flag);
	}
}
